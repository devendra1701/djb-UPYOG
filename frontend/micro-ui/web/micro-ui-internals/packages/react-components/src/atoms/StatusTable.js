          ...
          {props?.privacy && (
            <span style={{ display: "inline-flex", width: "fit-content", marginLeft: "10px" }}>
              ...
              */}
              ...
              <WrapUnMaskComponent
                value={val}
                iseyevisible={val?.includes("*") ? true : false}
                privacy={Array.isArray(props?.privacy) ? props?.privacy?.[index] : props?.privacy}
              />
            </span>
          )}
        </p>
      ...
      <div style={props.rowContainerStyle} className={`${props.last ? "row last" : "row"} ${props?.className || ""}`}>
        ...
        {props?.text?.map((ob, index) => (
          <div
            ...
            style={
              index == 0
                ? { ...valueStyle, wordBreak: "break-word", marginLeft: "28.5%", width: "20%" }
                : { ...valueStyle, wordBreak: "break-word", color: "grey", display: "inline", fontSize: "13px", paddingLeft: "10px" }
            }
          >
            <WrapUnMaskComponent
              value={ob?.value}
              iseyevisible={ob?.value && ob?.value?.toString()?.includes("*") ? true : false}
              privacy={props?.privacy?.[index]}
            />
    ...
    return (
      <div style={props.rowContainerStyle} className={`${props.last ? "row last" : "row"} ${props?.className || ""}`}>
        ...
        {props?.privacy && (
          ...
          <WrapUnMaskComponent
            value={value}
            iseyevisible={value && value?.toString()?.includes("*") ? true : false}
            privacy={Array.isArray(props?.privacy) ? props?.privacy?.[0] : props?.privacy}
          />
          {props.caption && <div className="caption">{props.caption}</div>}
        </div>
        {props.actionButton ? (
